...
const I18N = {
  zh: {
    ...
    ai_output_lang:  "繁體中文",
    tw_history:       "▌ 台海緊張指數趨勢（近30日）",
    tw_mnd_trend:     "▌ 解放軍動態趨勢（近30日）",
    tw_chart_no_data: "（歷史數據累積中，請稍後再查看）",
    tw_chart_loading: "載入中...",
    tw_gdelt:         "▌ GDELT 全球媒體聲量訊號（近30日）",
    tw_gdelt_vol:     "台灣軍事相關媒體聲量",
    tw_gdelt_tone:    "媒體情緒（負值=負面/緊張）",
    tw_gdelt_fail:    "GDELT 資料暫時無法取得",
    anomaly_up:       (delta, sigma) => `⚠ 異常升溫偵測　緊張指數較7日均值高 ${delta} 點（${sigma}σ）`,
    anomaly_down:     (delta, sigma) => `↓ 緊張程度顯著緩解　較7日均值低 ${Math.abs(delta)} 點（${sigma}σ）`,
    scenario_dynamic_note: "依當前緊張指數與 MND 數據動態調整（基準：灰55/封18/彈15/侵12）",
    tw_playback_label: "歷史快照",
    tw_playback_select:"選擇日期...",
    tw_playback_live:  "返回即時",
    tw_playback_badge: (date) => `📅 歷史快照 ${date}　—　僅顯示當日存檔數據`,
    tw_playback_no_data:"尚無歷史資料",
    tw_playback_events:"▌ 當日代表事件（存檔）",
    tw_early_warning_title:"▌ 多維預警雷達",
  },
  en: {
    ...
    tw_history:       "▌ Taiwan Tension Index Trend (30 Days)",
    tw_mnd_trend:     "▌ PLA Activity Trend (30 Days)",
    tw_chart_no_data: "(Historical data accumulating — check back later)",
    tw_chart_loading: "Loading...",
    tw_gdelt:         "▌ GDELT Global Media Volume Signal (30 Days)",
    tw_gdelt_vol:     "Taiwan military media volume",
    tw_gdelt_tone:    "Media tone (negative = alarming coverage)",
    tw_gdelt_fail:    "GDELT data temporarily unavailable",
    anomaly_up:       (delta, sigma) => `⚠ Anomalous Escalation　Tension index ${delta} pts above 7-day avg (${sigma}σ)`,
    anomaly_down:     (delta, sigma) => `↓ Significant De-escalation　${Math.abs(delta)} pts below 7-day avg (${sigma}σ)`,
    scenario_dynamic_note: "Dynamically adjusted based on current tension index & MND data (baseline: GZ55/BL18/MS15/FI12)",
    tw_playback_label: "Historical Snapshot",
    tw_playback_select:"Select date...",
    tw_playback_live:  "Back to Live",
    tw_playback_badge: (date) => `📅 Historical Snapshot ${date} — archived data only`,
    tw_playback_no_data:"No historical data yet",
    tw_playback_events:"▌ Representative Events (Archived)",
    tw_early_warning_title:"▌ Early Warning Radar",
  }
};
